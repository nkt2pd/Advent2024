...
int main() {
    ...
    while(!input.eof()) {
        ...
        //check for conditions for a safe report, removing 1 data point each time

        int report_pos = 0;
        int report_point_removed = 0;

        for(int i = 0; i < report_data.size(); i++) {
            report_point_removed = report_data[i];

            report_data.erase(report_data.begin() + i);
            ...
            report_data.emplace(report_data.begin() + i, report_point_removed);
        }
